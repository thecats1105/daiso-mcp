"""
이마트24 앱 요청 캡처 결과를 전달 가능한 JSONL/요약 파일로 저장하는 mitmproxy 애드온.

사용 예시:
  mitmdump -s scripts/mitmproxy/emart24_capture_export.py \
    --set emart24_capture_dir=captures/emart24-20260308 \
    --set emart24_capture_scenario='예약픽업 상품 검색 및 매장 선택' \
...
class Emart24CaptureExportAddon:
    ...
    def load(self, loader):
        loader.add_option(
            name='emart24_capture_dir',
            typespec=str,
            default='captures/emart24-latest',
            help='이마트24 캡처 산출물 저장 디렉토리',
        )
        loader.add_option(
            name='emart24_capture_scenario',
            typespec=str,
            default='미지정',
            help='수집 시나리오 설명',
        )
        loader.add_option(
            name='emart24_capture_hosts',
            typespec=str,
            default='emart24.co.kr,abr.ge',
    ...
    def running(self):
        ...
        ctx.log.info(f'[emart24-capture] 출력 디렉토리 초기화: {output_dir}')
    ...
    def done(self):
        summary = {
            ...
            'scenario': ctx.options.emart24_capture_scenario,
        ...
        ctx.log.info(f'[emart24-capture] 요약 파일 생성: {summary_path}')

    def _output_dir(self) -> Path:
        return Path(ctx.options.emart24_capture_dir).expanduser()
    ...
    def _target_hosts(self) -> list[str]:
        hosts = [item.strip().lower() for item in ctx.options.emart24_capture_hosts.split(',')]
...
addons = [Emart24CaptureExportAddon()]
